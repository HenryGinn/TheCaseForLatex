...
The cost of these improvements is that \LaTeX\ is less simple to use than Word, although I would argue the gap is not that large, especially for basic word processing. Table~\ref{tab:Difficulty} summarises how complex certain tasks are to do in \LaTeX\ with examples given below. The general principle of simple aims being simple to execute and complex aims being complex to execute holds, as long as it is inline with good typesetting principles. For example, writing text sideways might need some research.
...
\begin{table}[H]
	...
	\begin{tabular}{cp{110 mm}}
		...
		Single equations & Nearly as easy as Word, for example \Verb_\alpha_ for $\alpha$.  \\
		Complex equations & Will need some practice and around an hour of training.  \\
...
\subsection{Most Complicated Example}
\label{subsec:MostComplicatedExample}

Inserting images is likely the most difficult task most employees would need to do. The below code produces two subfigures side by side, although could easily be modified to have three. I posit that most WSP employees could deduce that they would need to add another subfigure block and lower the width.
